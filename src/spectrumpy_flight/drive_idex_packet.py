...
def find_source_files(roots: List[Path]) -> List[Path]:
    ...
    for root in roots:
        if not root.exists():
            continue
        for p in root.rglob("*"):
            if p.is_file() and p.suffix == "":   # “no extension” like your Bash find
                files.append(p)
    return files
...
def run_one(idx: int, py: str, script: Path, src: Path, env: dict, log_dir: Path, nice: int | None) -> Tuple[Path, int, float]:
    """Run `python idex_packet.py -f <src>`; return (src, rc, seconds)."""
    log_dir.mkdir(parents=True, exist_ok=True)
    out_log = log_dir / f"job_{idx:04d}.out"
    err_log = log_dir / f"job_{idx:04d}.err"

    cmd = [py, str(script), "-f", str(src)]
    start = time.time()
...
def main():
    ...
    ap.add_argument("--idex-script", default="idex_packet.py", help="Path to idex_packet.py")
    ...
    script = Path(args.idex_script).resolve()
    ...
    print(f"Running up to {max_workers} concurrent jobs "
          f"(threads-per-proc={args.threads_per_proc or 'default'})")

    ok, fail = 0, 0
    t0 = time.time()
    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        futs = [
            ex.submit(run_one, idx, args.python, script, src, env, log_dir, args.nice)
