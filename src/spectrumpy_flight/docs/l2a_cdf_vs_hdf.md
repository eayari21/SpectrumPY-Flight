...
produced by `tools/cdf_strings_report.py` (`src/spectrumpy_flight/docs/l2a_cdf_ascii_scan.md`) and the
...
* **Primary dimensions** – The CDF advertises an `epoch` record and two time
  ...
  steps).【F:src/spectrumpy_flight/docs/l2a_cdf_ascii_scan.md†L15-L38】
* **TOF family** – The quicklook TOF panels correspond to `tof_high`,
  ...
  in the HDF workflow.【F:src/spectrumpy_flight/docs/l2a_cdf_ascii_scan.md†L40-L68】
* **Target and Ion Grid fits** – For each low-rate channel the CDF includes the
  ...
  the dust impact on Ion Grid channel…”).【F:src/spectrumpy_flight/docs/l2a_cdf_ascii_scan.md†L75-L148】
* **Fit parameter dictionary** – `target_fit_parameter_index` and
  ...
  fit parameters (impact time, offset, amplitude, rise/decay constants).【F:src/spectrumpy_flight/docs/l2a_cdf_ascii_scan.md†L69-L80】
* **Derived axis labelling** – Although the ASCII scan yields raw tokens, the
  ...
  place.【F:src/spectrumpy_flight/docs/l2a_cdf_ascii_scan.md†L85-L148】
...
* Each impact is stored in its own group (`/{Number}/…`) containing the raw
  ...
  `Target L/H`, etc.) and reuse the same time column for all tracks.【F:src/spectrumpy_flight/ImpactBook.py†L163-L172】
* Derived products (fit arrays, scalar diagnostics, accelerator solution, etc.)
  ...
  (kmps, QD Fit)”).【F:src/spectrumpy_flight/ImpactBook.py†L228-L270】
...
2. **Waveform channels** – The TOF low/mid/high, Target low/high, and Ion Grid
   ...
   datasets created in quicklook (`/{Number}/TOF H`, `/Target H`, etc.).【F:src/spectrumpy_flight/docs/l2a_cdf_ascii_scan.md†L31-L133】【F:src/spectrumpy_flight/ImpactBook.py†L163-L172】
3. **Fit metadata** – The CDF fit products already provide both parameter
   ...
   entries map to their similarly named datasets in the HDF writer.【F:src/spectrumpy_flight/docs/l2a_cdf_ascii_scan.md†L82-L148】【F:src/spectrumpy_flight/ImpactBook.py†L228-L270】
...
4. **Validate with side-by-side plots** – Once the adapter is in place, compare
   ...
   (`src/spectrumpy_flight/docs/l2a_cdf_ascii_scan.md`) can be used as a checklist to confirm that all
