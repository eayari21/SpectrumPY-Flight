...
Under the standard constant-field TOF approximation the travel time for an ion with mass-to-charge ratio `(m / q)` is well approximated by the polynomial

`t(m) = t0 + sum_{k=1..K} a_k * (sqrt(m))^k`

Introducing the reduced coordinate `s = sqrt(m)` yields a series in `s` rather than directly in mass:

`t(s) = t0 + a1*s + a2*s^2 + ... + aK*s^K`

The coefficients `a_k` are determined from calibration data by solving a weighted least-squares system. Given `N` calibration lines (`N >= 2`), the design matrix `A` has entries `A[i, k] = s_i^k` for `s_i = sqrt(m_i)`.  The optimal coefficients solve the normal equations `(A^T W A) c = A^T W t`, where `W` is a diagonal weight matrix with elements `w_i = 1 / sigma_i^2` when measurement uncertainties `sigma_i` are known.  The `fit_tof_series` routine directly computes the least-squares solution via `numpy.linalg.lstsq`, which internally performs an SVD for numerical stability.
...
`s_{n+1} = clip(s_n - f(s_n) / f'(s_n), 0, s_max)`

with `f(s) = t(s) - t` and `f'(s) = dt(s)/ds = sum_{k=1..K} k * a_k * s^{k-1}`.  The iteration terminates when `abs(s_{n+1} - s_n) < epsilon` (default `1e-9`) or when a fixed number of iterations is reached.  After convergence the squared reduced coordinate gives the calibrated mass,

`m = (s_converged)^2`

which is finally clipped to the instrument-supported range `[m_min, m_max]`.
...
1. **Stretch search.**  Estimate the global scale factor `alpha` by scanning `alpha` within `[1.3, 1.6]` µs (configurable via `MASS_STRETCH_MIN_US`/`MAX_US`).  For each candidate the software rescales the time axis as `s = (t - Delta) / alpha` and evaluates the agreement with reference lines.
...
* **Residual analysis.**  Plot `t_i - t(m_i)` versus `sqrt(m_i)` to detect systematic deviations.  Random residuals with RMS under ~10 ns indicate a reliable calibration.
...
* **Clipping behavior.**  The implementation clips Newton steps to the instrument bounds to prevent runaway solutions.  If many samples saturate at `m_max` consider expanding the range or revisiting the initial stretch.
