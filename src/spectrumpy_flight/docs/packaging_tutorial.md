...
3. **Verify documentation assets.** Confirm that `README.md`, the `src/spectrumpy_flight/docs/`
...
5. **Run the Quicklook GUI once.** Execute `spectrumpy-quicklook` and open a
...
4. **First-launch verification.** From a non-development account, double-click
   ...
   HDF5 or CDF file.【F:src/spectrumpy_flight/spectrum_launcher.py†L167-L247】
...
1. **Launch through the Spectrum Launcher.** Double-click the packaged binary or
   ...
   * Display the IMAP/IDEX imagery and metadata.【F:src/spectrumpy_flight/spectrum_launcher.py†L124-L169】
   * Allow browsing for `.h5`, `.hdf5`, `.cdf`, or `.trc` files and enable the
     appropriate buttons once a file is chosen.【F:src/spectrumpy_flight/spectrum_launcher.py†L176-L247】
   * Open the HDF Plotter or IDEX Quicklook in separate windows with the selected
     dataset.【F:src/spectrumpy_flight/spectrum_launcher.py†L249-L289】
2. **Use the welcome screen shortcuts.** Test both buttons to ensure the child
   ...
   launcher unexpectedly.【F:src/spectrumpy_flight/spectrum_launcher.py†L249-L279】
3. **Verify documentation availability.** Open the Quicklook viewer, press `F1`,
   ...
   this playbook) thanks to the spec bundling the `src/spectrumpy_flight/docs/` directory.【F:packaging/idex_quicklook.spec†L21-L47】
...
Plotter or full Quicklook interface.【F:src/spectrumpy_flight/start.py†L1-L7】【F:src/spectrumpy_flight/spectrum_launcher.py†L167-L289】
...
* **File chooser defaults.** The launcher opens in the `HDF5/` directory if it
  ...
  files without restarting.【F:src/spectrumpy_flight/spectrum_launcher.py†L220-L247】
* **Multiple sessions.** Each viewer window registers itself with the launcher
  ...
  extra files.【F:src/spectrumpy_flight/spectrum_launcher.py†L249-L279】
...
* **Documentation center shows empty list.** Confirm the `src/spectrumpy_flight/docs/` directory was
