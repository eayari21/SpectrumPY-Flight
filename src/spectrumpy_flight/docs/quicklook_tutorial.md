...
* **Packaged desktop build.** Double-click the SpectrumPY bundle created with
  ...
  a data product before entering the viewer.【F:src/spectrumpy_flight/spectrum_launcher.py†L167-L289】
* **Source checkout (recommended for developers).** From the repository root run
  ...
  the Spectrum Launcher’s `main()` function.【F:src/spectrumpy_flight/start.py†L1-L7】
* **Direct module invocation.** Use `python spectrum_launcher.py` to skip the
  wrapper or `spectrumpy-quicklook --filename <file>` when you want to bypass
  the welcome screen entirely.【F:src/spectrumpy_flight/spectrum_launcher.py†L281-L293】【F:src/spectrumpy_flight/idex_quicklook.py†L1585-L1595】
...
1. Click **Browse…** to select `.h5`, `.hdf5`, `.cdf`, or `.trc` files. The file
   ...
   repository root.【F:src/spectrumpy_flight/spectrum_launcher.py†L220-L247】
2. Once a file is selected the launcher enables the **Open in HDF Plotter**
   ...
   extension.【F:src/spectrumpy_flight/spectrum_launcher.py†L196-L274】
3. Launchers keep running while child windows are open, so you can inspect
   ...
   welcome window.【F:src/spectrumpy_flight/spectrum_launcher.py†L249-L279】
...
2. Ensure the mission directory structure exists: `Data/`, `HDF5/`, `CDF/`, and
   ...
   opening dialogs and exporting results.【F:src/spectrumpy_flight/ImpactBook.py†L124-L143】【F:src/spectrumpy_flight/combine_target_signals.py†L30-L149】
3. Run `python -m imfpy.gui.main` if SpectrumPY is installed as a package. The
   ...
   directly or scripting a pre-selected dataset.【F:src/spectrumpy_flight/imfpy/gui/main.py†L8-L129】
...
* **Open…** (toolbar or `File → Open…`) provides a unified dialog for HDF5/CDF
  ...
  time you launch it.【F:src/spectrumpy_flight/idex_quicklook.py†L82-L120】
...
* Use **Reload** after running a new decode or fit cycle; the viewer rereads the
  active file and refreshes plots without rebuilding channel selections.【F:src/spectrumpy_flight/idex_quicklook.py†L1056-L1188】
...
* The event selector lives on the right side of the toolbar. Scroll or use the
  ...
  and timestamp metadata when available.【F:src/spectrumpy_flight/idex_quicklook.py†L1216-L1294】
...
* The toggle panel groups every waveform by detector. Buttons glow indigo when
  active, giving you immediate feedback on what is plotted.【F:src/spectrumpy_flight/idex_quicklook.py†L1296-L1349】
...
* `Edit Fit Parameters` opens an interactive dialog where you can tweak fit
  coefficients, apply overrides, and revert to the on-disk solution.【F:src/spectrumpy_flight/idex_quicklook.py†L1370-L1587】
...
the currently loaded event (`Ctrl+N`).【F:src/spectrumpy_flight/idex_quicklook.py†L1972-L2026】【F:src/spectrumpy_flight/idex_quicklook.py†L2068-L2142】
...
* **Amplitude histogram with Gaussian overlay.** The window automatically fits a
  ...
  coherent pickup.【F:src/spectrumpy_flight/noise_analysis.py†L270-L331】
* **Power spectrum.** A detrended FFT reveals narrowband tones and broadband
  ...
  possible.【F:src/spectrumpy_flight/noise_analysis.py†L333-L360】
* **Autocorrelation.** The normalized autocorrelation plot highlights
  ...
  peaks back to instrument oscillators.【F:src/spectrumpy_flight/noise_analysis.py†L362-L402】
...
events refreshes the plots without reloading from disk.【F:src/spectrumpy_flight/noise_analysis.py†L55-L221】【F:src/spectrumpy_flight/noise_analysis.py†L402-L451】 The
analysis gracefully handles empty or missing channels by clearing the plots and
status text until a populated waveform is selected.【F:src/spectrumpy_flight/noise_analysis.py†L304-L324】
...
tables.【F:src/spectrumpy_flight/idex_quicklook.py†L2068-L2142】【F:src/spectrumpy_flight/dust_composition.py†L3008-L3123】 Each
...
the window.【F:src/spectrumpy_flight/dust_composition.py†L1387-L1478】【F:src/spectrumpy_flight/line_shapes.py†L17-L126】 Shape
...
constants or weight lists for composite tails.【F:src/spectrumpy_flight/dust_composition.py†L1387-L1478】
...
so you can compare species contributions across events.【F:src/spectrumpy_flight/dust_composition.py†L3208-L3295】【F:src/spectrumpy_flight/dust_composition.py†L3336-L3361】
...
without leaving the workspace.【F:src/spectrumpy_flight/dust_composition.py†L3008-L3525】【F:src/spectrumpy_flight/dust_composition.py†L3526-L4023】 Exported tables persist the
...
budgeting.【F:src/spectrumpy_flight/dust_composition.py†L3438-L3484】【F:src/spectrumpy_flight/dust_composition.py†L3828-L4023】
...
* Click the question-mark **Help** button or press `F1` to launch the
  ...
  technical memos) and renders their content inline.【F:src/spectrumpy_flight/idex_quicklook.py†L200-L409】
...
* The menu bar also includes a quick search widget under `Help` so you can start
  typing without leaving the main window.【F:src/spectrumpy_flight/idex_quicklook.py†L1134-L1192】
...
* The UI requires a display server. On headless Linux nodes use
  `xvfb-run spectrumpy-quicklook` to virtualize an X server.【F:src/spectrumpy_flight/idex_quicklook.py†L1585-L1595】
* When the fit overlay diverges from expectations, use **Reset Fit Overrides** to
  ...
  cleared.【F:src/spectrumpy_flight/idex_quicklook.py†L1056-L1188】
